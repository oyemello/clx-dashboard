...
export async function GET() {
    try {
        ...
        // Map multiple keys to the same persona list for resilience
        ...
        rows.forEach((r: any) => {
            ...
            const personas = r.persona ? [r.persona] : [];
            
            const add = (key: string) => {
                if (!key) return;
                if (!map[key]) map[key] = [];
                if (!map[key].includes(r.persona)) map[key].push(r.persona);
            };
        ...
        console.error("Failed to fetch persona mappings", error);
